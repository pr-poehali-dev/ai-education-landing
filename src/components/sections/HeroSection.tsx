...
export default function HeroSection({ scrollToForm, scrollToSection }: HeroSectionProps) {
  ...
  return (
    <div className="relative overflow-hidden min-h-screen flex flex-col justify-center">
      ...
      {/* ── ГЛАВНАЯ СЕТКА ── */}
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-24 sm:pt-28 pb-12 sm:pb-20">

        {/* Верхняя полоска — бейджи */}
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-6 sm:mb-8">
          <span className="inline-flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-xs sm:text-sm font-bold rounded-full shadow-lg shadow-cyan-500/30">
          ...
          <span className="inline-flex items-center gap-1.5 px-4 py-2 bg-red-600/20 border-2 border-red-500/60 text-red-300 text-xs sm:text-sm font-bold rounded-full animate-soft-pulse">
            ...
            На май осталось 2 места
          </span>
        </div>

        {/* Десктоп: 2 колонки 62/38 (≈ золотое сечение) | Мобайл: стопка */}
        <div className="grid lg:grid-cols-[62fr_38fr] gap-8 lg:gap-12 items-center">
          ...
          <div className="flex flex-col gap-6 sm:gap-7">
            ...
            {/* 3. Ключевые бейджи-факты */}
            <div className="flex flex-wrap gap-2 sm:gap-3">
              <div className="inline-flex items-center gap-2 px-4 py-2.5 sm:px-5 sm:py-3 bg-gradient-to-r from-orange-500 via-red-600 to-pink-600 text-white text-sm sm:text-base font-bold rounded-xl shadow-xl animate-soft-pulse border-2 border-orange-300/40">
                <Icon name="Users" size={18} className="animate-bounce" />
                ЖИВОЕ ОБУЧЕНИЕ В КЛАССЕ
              </div>
              <div className="inline-flex items-center gap-2 px-4 py-2.5 sm:px-5 sm:py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-sm sm:text-base font-bold rounded-xl shadow-lg border-2 border-green-300/40">
                <Icon name="GraduationCap" size={18} />
                С НУЛЯ — БЕЗ ОПЫТА
              </div>
            </div>

            {/* 4. Описание — конкретные результаты */}
            ...
            <div className="relative">
              ...
              <div className={`relative bg-slate-900/80 backdrop-blur-sm rounded-2xl p-5 border-2 ${isExpired ? 'border-green-500/40' : 'border-orange-500/40'} shadow-2xl`}>
                {isExpired ? (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <p className="text-orange-400 font-bold text-sm uppercase tracking-wider shrink-0">До старта:</p>
                    <div className="grid grid-cols-4 gap-2 flex-1">
                      {[
                        ...
                        <div key={label} className="text-center bg-orange-500/10 border border-orange-500/25 rounded-xl py-2.5 px-1">
                          <div className="text-xl sm:text-3xl font-bold text-white font-mono tabular-nums">
                            {String(val).padStart(2, '0')}
                          </div>
                          <div className="text-xs text-orange-300 font-medium mt-0.5">{label}</div>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex flex-col gap-4 pt-1">
              <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-sm text-gray-400">
                <span className="flex items-center gap-1.5"><Icon name="Users" size={15} className="text-cyan-400" />10 000+ студентов</span>
                <span className="flex items-center gap-1.5"><Icon name="Star" size={15} className="text-yellow-400" />Рейтинг 4.9/5</span>
                <span className="flex items-center gap-1.5"><Icon name="Clock" size={15} className="text-cyan-400" />3 месяца</span>
              </div>
